...
func _build() -> void:
	...
	frame.add_theme_stylebox_override("panel", StyleBoxEmpty.new() if str(data.get("_ui_context", "")) in ["player_board", "enemy_board"] else _frame_style())
	add_child(frame)
	...
	var art_x: float = 6.0 if _bf else 8.0
	var art_w: float = 130.0 if _bf else (custom_minimum_size.x - 16.0)
	var art_h: float = 130.0 if _bf else (82.0 if not compact else 64.0)
	art_clip = Panel.new()
	art_clip.position = Vector2(art_x, 2.0 if _bf else art_top)
	...
	art_rect.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_CENTERED if _bf else TextureRect.STRETCH_KEEP_ASPECT_COVERED
	if _bf:
		var battle_shader := Shader.new()
...
func summon_animation() -> void:
	...
	var drop_h  := 220.0 + tier * 35.0
	var drop_d  := 0.55 if tier >= 4 else 0.62

	# Start: high above, tilted, semi-transparent
	position.y = land_y - drop_h
	scale = Vector2(0.72 + tier * 0.035, 0.72 + tier * 0.035)
	rotation   = randf_range(-0.12, 0.12)
	modulate = Color(2.0 + tier * 0.25, 2.0 + tier * 0.25, 2.0 + tier * 0.25, 0.0)
	...
	slam.tween_property(self, "scale", Vector2(1.22 + tier * 0.04, 0.78 - tier * 0.03), drop_d)
	slam.tween_property(self, "rotation",    0.0,              drop_d * 0.7)
	var summon_flash: Color = _faction_flash_color()
	if tier >= 6:
			summon_flash = Color(0.35, 0.90, 1.0, 1.0)
	elif tier >= 5:
		summon_flash = Color(1.0, 0.28, 0.08, 1.0)
	elif tier >= 4:
		summon_flash = Color(0.65, 0.30, 1.0, 1.0)
	elif tier >= 3:
		summon_flash = Color(1.0, 0.72, 0.12, 1.0)
	elif tier >= 2:
		summon_flash = Color(0.82, 0.88, 0.95, 1.0)
	
	slam.tween_property(self, "modulate", summon_flash, drop_d * 0.45)
